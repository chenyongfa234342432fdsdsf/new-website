 ...
 * @param isRound 四舍五入
 * @param showNegativeNumbers 是否显示负数，不显示负数的话替代为 --
 * @returns
 */
 ...
 * 格式化负数为 '--'
 */
export function formatNegativeNumbers(data) {
 ...
 * 格式化 undefine 为 '--'
 */
export function formatNumbers(data, digits?: number) {
  return data ? formatCurrency(data, digits) : '--'
}
